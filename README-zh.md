...
[![Test](https://img.shields.io/badge/tests-233%20passed-brightgreen.svg)](#测试)
[![TDD](https://img.shields.io/badge/TDD-driven-red.svg)](#开发规范)

> 面向内部技术支持团队的企业级 AI Agent 平台

**[English](README-en.md)** · [返回主文档](README.md)
...
│   │   │       ├── ticket.py            #   工单 API
│   │   │       ├── auth.py              #   认证 API
│   │   │       ├── external.py          #   外部集成 API
│   │   │       ├── notification.py      #   IM 通知 API
│   │   │       └── admin.py             #   管理后台 API
│   │   ├── core/                         # 核心配置
│   │   │   └── config.py               #   全局设置
│   │   ├── db/                           # 数据库层
...
│   │   │   ├── user.py                 #   用户模型
│   │   │   ├── tenant.py               #   租户模型
│   │   │   ├── diagnosis_flow.py       #   诊断流程模型
│   │   │   └── audit_log.py            #   审计日志模型
│   │   ├── services/                     # 业务服务
│   │   │   ├── knowledge.py            #   知识库服务
│   │   │   ├── llm.py                  #   LLM 路由
│   │   │   ├── llm_fallback.py         #   多模型 Fallback
│   │   │   ├── chunking.py             #   文本分块
│   │   │   ├── rag.py                  #   RAG 检索
│   │   │   ├── diagnosis.py            #   错误诊断
│   │   │   ├── ticket.py               #   工单服务
...
│   │   │   ├── tenant.py               #   租户隔离服务
│   │   │   ├── audit.py                #   审计日志服务
│   │   │   ├── external_ticket.py      #   外部工单集成
│   │   │   ├── notification.py         #   IM 通知服务
│   │   │   ├── quality_eval.py         #   质量评估服务
│   │   │   └── knowledge_discovery.py  #   知识自动发现
│   │   ├── main.py                       #   应用入口
│   │   └── ...
│   ├── tests/                            # 测试套件
...
│   │   │   ├── KnowledgeDiscoveryPage.tsx #  知识发现
│   │   │   └── DiagnosisFlowPage.tsx   #     诊断流程编辑器
│   │   ├── services/                     #   API 服务
│   │   ├── store/                        #   Redux Store
│   │   ├── hooks/                        #   自定义 Hooks
│   │   ├── __tests__/                    #   前端测试
...
| **外部集成** |                                              |                               |
| POST         | `/api/v1/external/tickets/sync`              | 同步工单到外部系统            |
| GET          | `/api/v1/external/tickets/config/{provider}` | 获取外部系统配置              |
| **IM 通知**  |                                              |                               |
| POST         | `/api/v1/notification/send`                  | 发送 IM 通知                  |
| POST         | `/api/v1/notification/escalation`            | 发送升级通知                  |
| **管理后台** |                                              |                               |
| GET          | `/api/v1/admin/stats`                        | 系统统计                      |
| POST         | `/api/v1/admin/quality/evaluate`             | 质量评估                      |
| POST         | `/api/v1/admin/knowledge/discover`           | 知识自动发现                  |
...
| API 测试   | `tests/test_api/`              | 聊天、知识库、认证 API    |
| 数据库测试 | `tests/test_db/`               | 数据模型、会话            |
| 服务测试   | `tests/test_services/`         | LLM 路由、知识库、租户    |
